...
if [ $? == 0 ]
then
  podman run --rm -ti --name keyhole -v keyhole_vol:/usr/src/app/.keyhole/ keyhole:0.1.3
  echo ""
  exit 0
else
...
if [ $? == 0 ]
then
  docker run --rm -ti --name keyhole -v keyhole_vol:/usr/src/app/.keyhole/ keyhole:0.1.3
else
